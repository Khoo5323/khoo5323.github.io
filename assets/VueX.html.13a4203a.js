import{_ as e,o as i,c as n,a as s}from"./app.74a5dc6f.js";const t="/assets/Untitled.cf36d2b1.png",l={},d=s('<h1 id="vuex" tabindex="-1"><a class="header-anchor" href="#vuex" aria-hidden="true">#</a> VueX</h1><h1 id="vuex-状态管理器" tabindex="-1"><a class="header-anchor" href="#vuex-状态管理器" aria-hidden="true">#</a> <strong>Vuex 状态管理器</strong></h1><h1 id="一、vuex-状态管理器" tabindex="-1"><a class="header-anchor" href="#一、vuex-状态管理器" aria-hidden="true">#</a> <strong>一、Vuex 状态管理器</strong></h1><p><strong><strong>Vuex的原理</strong></strong></p><blockquote><p>概念：Vuex 是一个专为 Vue.js 应用程序开发的状态管理模式。它采用集中式存储管理应用的所有组件的状态，并以相应的规则保证状态以一种可预测的方式发生变化。</p></blockquote><p>应用场景：Vue多个组件之间需要共享数据或状态。</p><p>Vuex有几个核心属性：State、Getter、Mutation、Action、Module。</p><p><img src="'+t+`" alt="Untitled"></p><ul><li>State：存储状态数据</li><li>Getter：从状态数据派生数据，相当于State的计算属性。</li><li>Mutation：存储用于同步更改状态数据的方法，默认传入的参数为state。</li><li>Action：存储用于异步更改状态数据，但不是直接更改，而是通过触发Mutation方法实现，默认参数为context。</li><li>Module：Vuex模块化。</li></ul><p><strong><strong>全局配置Vuex</strong></strong></p><p>在 src 目录下创建 store 文件夹,并在里面创建一个index.js文件，然后index.js中配置如下：</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>// 第一步：引入Vue、和Vuex(固定写法)
import Vue from &#39;vue&#39;
